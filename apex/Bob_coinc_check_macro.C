...
//  runnumber = run.Atoi();
  ...
  for(Int_t i = 0; i <= (n_runs); i++){
    ...
    if (online == 1) {
//        ROOTFILE_DIR = "Rootfiles/apex_online_%d.root";
    filenamebase = Form("Rootfiles/apex_online_%d.root",runnumber+i);
    }
    else {
//        ROOTFILE_DIR = "Rootfiles/apex_%d.root";
    filenamebase = Form("Rootfiles/apex_%d.root",runnumber+i);
...
leg->AddEntry((TObject*)0,Form("Number of good T2 (T6 trigger) = %0.1f Million",n_good_coins/1000000.0),"");
...
leg->AddEntry((TObject*)0,Form("Total T6 triggers = %0.1f Million",n_total_T6/1000000.0),"");
leg->AddEntry((TObject*)0,Form("Percent coincidences good = %0.1f",100.0*ratio_good_to_T6),"");
leg->Draw();
ofstream file_out1;
ofstream file_out2;
ofstream file_out3;
file_out1<<runnumber<<","<<n_good_coins<<",0.0"<<endl;
file_out2<<runnumber<<","<<n_total_T6<<",0.0"<<endl;
file_out3<<runnumber<<","<<100.0*ratio_good_to_T6<<",0.0"<<endl;
file_out1.open("Per_run_N_good_coincidences.csv",std::ofstream::app);
file_out2.open("Per_run_N_total_coincidences.csv",std::ofstream::app);
file_out3.open("Per_run_percent_good_coincidences.csv",std::ofstream::app);
