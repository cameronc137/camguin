...
*/
  ...
  if (runnumber >= 0 && runnumber < 4369){
    t_min = 1685;
    t_max = 1710;
  }
  if (runnumber >= 4369 && runnumber < 10000){
    t_min = 1695;
    t_max = 1715;
  }
...
//if (flag==2){
	c1->cd(2);
	TH2F *ht2 = new TH2F(Form("ht2","R-arm tgt delta p vs R-arm tgt delta p, with Cherenkov > 2000 cut, timing cuts %f to %f",t_min,t_max),120,-0.06,0.06,60,-0.06,0.06); // should fall at 50 kHz / uA, but give large bins for safety
	...
	tree2->Draw(Form("R.tr.tg_dp:L.tr.tg_dp>>ht2","R.cer.asum_c>2000 && L.cer.asum_c>2000 && L.tr.n==1 && R.tr.n==1 && DR.rrawt2>%f && DR.rrawt2<%f && (DR.evtypebits>>6)&1",t_min,t_max),"colz");
	...
	file_out3.open("Per_run_delta_correlation_cerenkov_weaker_cuts.csv",std::ofstream::app);
        TH2F *ht3 = new TH2F(Form("ht3","L-arm tgt delta p vs R-arm tgt delta p, with Cherenkov > 1000 cut, timing cuts %f to %f",t_min,t_max),120,-0.06,0.06,60,-0.06,0.06);
        ...
        tree2->Draw(Form("R.tr.tg_dp:L.tr.tg_dp>>ht3","R.cer.asum_c>1000&&R.tr.n==1&&L.cer.asum_c>1000&&L.tr.n==1&&((DR.rrawt2>%f)&&(DR.rrawt2<%f))&&fEvtHdr.fEvtType==6",t_min,t_max),"COLZ");
	...
	TH1F *ht5 = new TH1F(Form("ht5","Sum of L-arm tgt delta p and R-arm tgt delta p, Cherenkov > 1000, timing cuts %f to %f",t_min,t_max),240,-0.12,0.12);
	...
	// ht1->SetMarkerStyle(3);ht1->SetMarkerSize(0.75);
	// &&(R.ps.e+R.sh.e)/(R.gold.p*1000)>0.7 &&(L.prl1.e+L.prl2.e)/(L.gold.p*1000)>0.7&
	tree2->Draw(Form("R.tr.tg_dp+L.tr.tg_dp>>ht5","R.cer.asum_c>1000&&R.tr.n==1&&L.cer.asum_c>1000&&L.tr.n==1&&((DR.rrawt2>%f)&&(DR.rrawt2<%f))&&fEvtHdr.fEvtType==6",t_min,t_max),"COLZ");
